...
let dataOne;
if (localStorage.crid != null) {
    dataOne = JSON.parse(localStorage.crid);
} else {
    dataOne = [];
}
add.onclick = function() {
    let newData = {
        ...
        cardCVVText: cardCVVText.innerHTML,
        cardExpiration: cardExpiration.value,
    };
    dataOne.push(newData);
    localStorage.setItem('crid', JSON.stringify(dataOne));
    console.log(dataOne);
